      ...
      <div class="flex" style="padding-top:10px">
        <div class="user-info">
          <div class="fontFamily dp1">
            <h1 style="font-size: 42px;">{{ categoryName }}</h1>
          </div>
        </div>
      </div>
    </div>
...
import WaterfallSlot from 'vue-waterfall/lib/waterfall-slot'
    ...
    export default {
        ...
        methods: {
            getPins () {
                let self = this
                let categoryName1 = self.$route.path.split("/")
                self.categoryName = categoryName1[2]
                let formData = new FormData()
                formData.append("id", self.arrCookie)
                formData.append("category", self.categoryName)
